...
#define secret_key_name_for_peer_a(public_key, to_free) \
	nm_construct_name_a ("peers.%s.preshared-key", (public_key), (to_free))

#define secret_key_name_for_peer_dup(public_key) \
	g_strdup_printf ("peers.%s.preshared-key", (public_key))
...
_peers_get_by_public_key (NMSettingWireGuardPrivate *priv,
                          const char *public_key,
                          gboolean public_key_is_normalized)
{
            ...
            gboolean check_same_key)
{
	...
	if (check_same_key) {
		pd_same_key = _peers_get_by_public_key (priv, public_key, TRUE);
		if (pd_same_key) {
			if (pd_idx) {
		...
		nm_assert (_peers_get_by_public_key (priv, public_key, TRUE) == pd_idx);
                        ...
                        NMConnectionSerializationFlags flags)
{
	...
	for (i_peer = 0; i_peer < n_peers; i_peer++) {
		...
		g_variant_builder_add (&builder, "{sv}", "public-key", g_variant_new_string (peer->public_key));

		if (   !NM_FLAGS_HAS (flags, NM_CONNECTION_SERIALIZE_ONLY_SECRETS)
		    && peer->endpoint)
			g_variant_builder_add (&builder, "{sv}", "endpoint", g_variant_new_string (nm_sock_addr_endpoint_get_endpoint (peer->endpoint)));

		if (   !NM_FLAGS_HAS (flags, NM_CONNECTION_SERIALIZE_NO_SECRETS)
		    && peer->preshared_key)
			g_variant_builder_add (&builder, "{sv}", "preshared-key", g_variant_new_string (peer->preshared_key));

		if (   !NM_FLAGS_HAS (flags, NM_CONNECTION_SERIALIZE_ONLY_SECRETS)
		    && peer->preshared_key_flags != NM_SETTING_SECRET_FLAG_NOT_REQUIRED)
			g_variant_builder_add (&builder, "{sv}", "preshared-key-flags", g_variant_new_uint32 (peer->preshared_key_flags));

		if (   !NM_FLAGS_HAS (flags, NM_CONNECTION_SERIALIZE_ONLY_SECRETS)
		    && peer->persistent_keepalive != 0)
			g_variant_builder_add (&builder, "{sv}", "persistent-keepalive", g_variant_new_uint32 (peer->persistent_keepalive));
		...
		if (   !NM_FLAGS_HAS (flags, NM_CONNECTION_SERIALIZE_ONLY_SECRETS)
			...
			g_variant_builder_add (&builder, "{sv}", "allowed-ips",
                      ...
                      GError       **error)
{
	...
	while (g_variant_iter_next (&iter_peers, "@a{sv}", &peer_var)) {
		...
		if (!g_variant_lookup (peer_var, "public-key", "&s", &cstr)) {
		...
		if (g_variant_lookup (peer_var, "endpoint", "&s", &cstr)) {
		...
		if (g_variant_lookup (peer_var, "preshared-key", "&s", &cstr))
			nm_wireguard_peer_set_preshared_key (peer, cstr);

		if (g_variant_lookup (peer_var, "preshared-key-flags", "u", &u32))
			nm_wireguard_peer_set_preshared_key_flags (peer, u32);

		if (g_variant_lookup (peer_var, "persistent-keepalive", "u", &u32))
			nm_wireguard_peer_set_persistent_keepalive (peer, u32);

		if (g_variant_lookup (peer_var, "allowed-ips", "@as", &var)) {
...
need_secrets (NMSetting *setting)
{
	...
	for (i = 0; i < priv->peers_arr->len; i++) {
		...
		if (!secrets)
			secrets = g_ptr_array_new_full (1, g_free);
		g_ptr_array_add (secrets, secret_key_name_for_peer_dup (peer->public_key));
	}
	...
	if (nm_streq (sett_info->property_infos[property_idx].name, NM_SETTING_WIREGUARD_PEERS)) {
		...
		for (i = 0; i < priv->peers_arr->len; i++) {
			...
			if (func) {
				...
				if (j++ < 5)
					name = secret_key_name_for_peer_a (peer->public_key, &name_free);
				else {
					name_free = secret_key_name_for_peer_dup (peer->public_key);
					name = name_free;
				}
