...
static gboolean
_setting_release_hrf (gpointer key, gpointer value, gpointer user_data)
{
                                 ...
                                 GError **error)
{
	...
	if (g_hash_table_size (priv->settings) > 0) {
		g_hash_table_foreach_remove (priv->settings, _setting_release_hrf, connection);
		changed = TRUE;
	} else
...
nm_connection_private_free (NMConnectionPrivate *priv)
{
	...
	g_hash_table_foreach_remove (priv->settings, _setting_release_hrf, self);
